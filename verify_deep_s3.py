...
tsv_path = 'train_source3.tsv'
pq_path = 'intermediate/source3_normalized.parquet'
...
print("\n--- 6. Investigate NULL Addresses in Source 3 ---")
null_addr = con.execute("SELECT COUNT(*) FROM tsv_data WHERE business_address IS NULL").fetchone()[0]
literal_bracket_null = con.execute("SELECT COUNT(*) FROM tsv_data WHERE business_address LIKE '%<NULL>%'").fetchone()[0]
literal_null = con.execute("SELECT COUNT(*) FROM tsv_data WHERE LOWER(business_address) LIKE '%null%' AND business_address NOT LIKE '%<NULL>%'").fetchone()[0]
print(f"SQL NULL addresses: {null_addr}")
print(f"Literal '<NULL>' addresses: {literal_bracket_null}")
print(f"Literal 'null' addresses (no brackets): {literal_null}")
...
print("\n--- 7. Execution Stats ---")
print(f"Output Parquet Size: {pq_size_mb:.2f} MB")
