...
def main():
    print("Starting Final Blocking Evaluation...")
    ...
    con.execute("CREATE TABLE cands AS SELECT * FROM read_parquet('intermediate/blocking_final_candidates.parquet')")
    ...
    with open('reports/blocking_final_evaluation.txt', 'w', encoding='utf-8') as f:
        f.write("BLOCKING FINAL EVALUATION REPORT\n")
        f.write("================================\n")
        ...
        f.write("--- FINAL UNION of All Strategies ---\n")
    ...
    print(f"Evaluation Complete. Check reports/blocking_final_evaluation.txt. Time: {time.time() - start_time:.2f}s")
