...
def main():
    print("Starting Refined Candidate Generation...")
    ...
    print("Precomputing frequencies for S2+S3...")
    con.execute("""
    ...
    def get_queries(s2_table, s2_nt, s2_at, source_name):
        return [
            ...
            ("rare_name_token_country", f"SELECT s1.entity_id, s2.entity_id, '{source_name}', 'rare_name_token_country' FROM s1_nt s1 JOIN {s2_nt} s2 ON s1.token = s2.token AND s1.country_clean = s2.country_clean"),
    ...
    con.execute("COPY dedup_candidates TO 'intermediate/blocking_refined_candidates.parquet' (FORMAT PARQUET)")
