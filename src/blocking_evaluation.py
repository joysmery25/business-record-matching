...
def main():
    print("Starting Blocking Evaluation...")
    ...
    print(f"Total True Matches for sample: {total_true} (S2: {total_true_s2}, S3: {total_true_s3})")

    if total_true == 0:
        print("ERROR: No ground truth found for the 10,000 S1 sample.")
        return

    # 2. Load generated candidates
    con.execute("CREATE TABLE cands AS SELECT * FROM read_parquet('intermediate/blocking_sample_candidates.parquet')")
    ...
    with open('reports/blocking_evaluation.txt', 'w', encoding='utf-8') as f:
        f.write("BLOCKING EVALUATION REPORT\n")
        f.write("==========================\n")
        f.write("Dataset/subset used: 10,000 random S1 records\n")
        ...
        f.write("Which strategies should be kept: Those with high recall and low average/max candidates.\n")
        f.write("Which strategies should be modified/removed: Those yielding enormous max candidates (e.g. >10,000 per S1), meaning they triggered on common stopwords.\n")
        f.write("Recommended next step: Proceed with full candidate generation using the surviving strategies, then compute Pair Features.\n")
    ...
    print(f"Evaluation Complete. Check reports/blocking_evaluation.txt. Time: {time.time() - start_time:.2f}s")
